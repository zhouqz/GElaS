...
#Comparison list
comg=c("Young_FastvsYoung_AdLibitum","Young_RefeedvsYoung_Fast",
       "Aged_FastvsAged_AdLibitum","Aged_RefeedvsAged_Fast")

#Young and Aged sample groups
YoungSample=c("Young_AdLibitum","Young_Fast","Young_Refeed")
AgedSample=c("Aged_AdLibitum","Aged_Fast","Aged_Refeed")


#*******************************************************************************#
#Input the expression dynamics and statistical significance information
#Note: The expression dynamics and statistical significance information 
#can be obtained for RNA-seq data by edgeR, limma, DESeq.2, etc.
for(tissi in TissTitle){
  tTissExp=fread(file = paste0(tissi,"/Young.Aged.Gene.Expression.txt"),sep = "\t",data.table = F)
  rownames(tTissExp)=tTissExp$Geneid
  assign(paste0(tissi,"Exp"),tTissExp)
  
  for(tcomgi in comg){
    tFCFDR=read.table(paste0(tissi,"/",tcomgi,".log2FC.FDR.txt"),sep = "\t",header = T)
...
for(tissi in TissTitle){
  ...
  tTissExp=get(paste0(tissi,"Exp"))
  tgid=tTissExp$Geneid
  
  #Calculate the GElaS in Young and Aged mice
  for(tagei in c("Young","Aged")){
    YASample=get(paste0(tagei,"Sample"))
    #Obtain the expression matrix
    tScoreMat=tTissExp[,c("Geneid","Symbol",paste0(YASample,".log2fpkm.mean"))]
    
    #Obtain the expression dynamics
