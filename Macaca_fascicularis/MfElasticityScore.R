...
#Note: Here, we perform the paired RNA-seq analysis for monkey by limma. 
#In general, The expression dynamics and statistical significance information 
#can be obtained for RNA-seq data by edgeR, limma, DESeq.2, etc.

for(tissi in TissTitle){
  tTissExp=fread(file = paste0(tissi,"/Gene.Expression.txt"),sep = "\t",data.table = F)
...
for(tissi in TissTitle){
  ...
  tSample=c("AdLibitum","Fast","Refeed")
  tScoreMat=tTissExp[,c("Geneid","Symbol",paste0(tSample,".log2fpkm.mean"))]
  
  #Obtain the expression dynamics
  tFN.name=paste0(tissi,".FastvsAdLibitum.FCFDR")
  tRF.name=paste0(tissi,".RefeedvsFast.FCFDR")
  ...
  tScoreMat[,paste0("GElaS")]=data.frame(tGElaS,stringsAsFactors = F)
  
  #Output the GElaS
  fwrite(tScoreMat,file = paste0(tissi,"/GElaS.txt"),sep = "\t")
}
